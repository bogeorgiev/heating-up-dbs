...
if __name__=="__main__":
    ...
    for i in range(1000):
        ...
        if (i+1) % 5 == 0:
            np.save("../data/cap_vol_stats/vol_data_noisy0.1", np.array(vol_data))
            np.save("../data/cap_vol_stats/cap_data_noisy0.1", np.array(cap_data))
            #np.save("dist_data_2", np.array(dist_data))
            np.save("../data/cap_vol_stats/radius_data_0.1", np.array(radius_data))
